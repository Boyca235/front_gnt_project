...
export default function PostDetailsTwo() {
  ...
  useEffect(() => {
    ...
    const fetchData = fectchPublication(id);
    fetchData({ id })
  }, [])
  const fectchPublication = id => async ({id}) => {
    try {
      await axios.get(`/publishment/find/${id}`).then(({ data }) => {
        setPublications(data)
        kat = data[0].category.category_name
      })
    } catch (error) {
          ...
          {postData.map((item, i) => (
            <div className="container" key={i + 1}>
              <div className="row justify-content-lg-center">
                <div className="col-8">
                  <BreadCrumb CategoryName={item.category.category_name} />
                </div>
              </div>
              <div className="row justify-content-center">
                <div className="col-lg-8">
                  <div className="post-layout-top-content post-layout-top-content-2" key={i + 1}>
                    <div className="thumb">
                    ...
                    <div className="post-author">
                      <div className="author-info">
                        ...
                        <h5 className="title">{item.publisher ? item.publisher.name : 'La rédaction'}</h5>
                        <ul>
                    ...
                    <div className="post-content">
                      <h3 className="title">
                        {item.category.category_name.toUpperCase()}
                      </h3>
                    </div>
